package it.micheledichio.moneytransfers.test.route.accounts;
...
import it.micheledichio.moneytransfers.route.accounts.PutAccountRoute;
import it.micheledichio.moneytransfers.service.AccountAbstractService;

public class PutAccountRouteTest {
    ...
    public void anEmptyBodyReturnsBadRequest() {
        ...
        PutAccountRoute route = new PutAccountRoute(service);
	...
	@Test
    public void anAccountNotInRepositoryReturnsNotFound() {
        Account newAccount = new Account();
        newAccount.setNumber("123456789");
        newAccount.setCurrency("EUR");
        newAccount.setBalance(new BigDecimal("1500"));
        ...
        AccountAbstractService service = EasyMock.createMock(AccountAbstractService.class);
        expect(service.update(newAccount)).andReturn(null);
        replay(service);

        PutAccountRoute route = new PutAccountRoute(service);
        assertEquals(new Answer(HttpStatus.NOT_FOUND_404, gson.toJson(new ApiError(HttpStatus.NOT_FOUND_404, "Account not found"))), route.process(newAccount, Collections.emptyMap()));

        verify(service);
    }
	
	@Test
    public void anAccountIsCorrectlyUpdatedAndReturnsOk() {
        Account newAccount = new Account();
        newAccount.setNumber("123456789");
        newAccount.setCurrency("EUR");
        newAccount.setBalance(new BigDecimal("1500"));
        ...
        AccountAbstractService service = EasyMock.createMock(AccountAbstractService.class);
        expect(service.update(newAccount)).andReturn(newAccount);
        replay(service);
